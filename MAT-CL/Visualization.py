...
def Data_prepared(n_classes):
    X_train_labeled, X_train_unlabeled, X_train, X_val, value_Y_train_labeled, value_Y_train_unlabeled, value_Y_train, value_Y_val = TrainDataset(n_classes,300)
...
def main():
    ...
    model = torch.load('model_weight/CNN_MAT1_n_classes_10_10label_90unlabel_rand30_autoweight100.pth',map_location=torch.device('cpu'))
    ...
    visualize_data(X_test_embedding_feature_map, target.astype('int64'),
                   "Visualization/SimMIM_encoder_mask05_n_classes_10_label10_improved", 10)
    print(sm.silhouette_score(X_test_embedding_feature_map, target, sample_size=len(X_test_embedding_feature_map),
                              metric='euclidean'))
