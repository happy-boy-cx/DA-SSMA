...
def Data_prepared(n_classes, rand_num):
    X_train_labeled, X_train_unlabeled, X_train, X_val, value_Y_train_labeled, value_Y_train_unlabeled, value_Y_train, value_Y_val = TrainDatasetx(n_classes, rand_num)
...
def main():
    ...
    # 加载模型
    model = torch.load("model_weight/CNN_DA_SSMA_classes_10_20label_80unlabel_rand30.pth", map_location=torch.device('cpu'))
    # 测试不同SNR
    ...
    df.to_csv("accuracy_vs_snr_20.csv", index=False)
    ...
    plt.savefig("snr_accuracy_curve_20.png", dpi=300, bbox_inches='tight')
    plt.show()
